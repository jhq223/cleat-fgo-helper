...
/// Encrypt plain text → base64 script text.
pub fn encrypt(
    plain_text: &str,
    stage_data: &[u8; 32],
    stage_top: &[u8; 32],
    ...
    // Compress
    let compressed = if use_bzip2 {
        let mut comp = Vec::new();
        let mut encoder = bzip2::write::BzEncoder::new(&mut comp, bzip2::Compression::best());
        encoder
            .write_all(&inverted)
            .map_err(|e| format!("bzip2 compress: {e}"))?;
        encoder.finish().map_err(|e| format!("bzip2 finish: {e}"))?;
        comp
    } else {
        let mut comp = Vec::new();
        let mut encoder = flate2::write::GzEncoder::new(&mut comp, flate2::Compression::best());
        encoder
            .write_all(&inverted)
